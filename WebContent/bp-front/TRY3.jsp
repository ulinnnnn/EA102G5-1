					...
					<div class="panel panel-default">
						...
						<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/bp-front/BpServlet" enctype="multipart/form-data">
								<div class="form-group">
									...
									value="<%=bpVO.getBp_head()%>" class="form-control" id="" style="border-radius: 15px !important">
								</div>
								<div class="form-group">
									<label for="">商品價格</label>
									<input type="TEXT" name="bp_pri" size="45"
									value="<%=bpVO.getBp_pri()%>"class="form-control" id="" style="border-radius: 15px !important">
								</div>
								<div class="form-group">
									<label for="">運送方式</label> 
									<input type="TEXT" name="bp_del" size="45"
									 value="<%=bpVO.getBp_del()%>"class="form-control" id="" style="border-radius: 15px !important">
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<label for="">文字</label> 
								<textarea class="form-control" name="bp_text" style="border-radius: 15px !important">${(bpVO == null)?"":bpVO.bp_text}</textarea>
								</div>
								
								<div class="form-group">
								<label for="">商品類別</label>
								<jsp:useBean id="bpclassService" scope="page" class="com.bpclass.model.BpclassService" />
									<select size="1" name="bpcl_no" class="form-control" style="border-radius: 15px !important">
										<c:forEach var="bpclassVO" items="${bpclassService.all}">	
											<option  style="border-radius: 15px !important" value="${bpclassVO.bpcl_no}" ${(bpVO.bpcl_no == bpclassVO.bpcl_no)?'selected':''}>${bpclassVO.bpcl_name}
										</c:forEach>
									</select>				
							</div>
							...
							<div class="form-group">
								...
								<label><input class="rad" name="bp_new" type="radio"  value="全新" <%= ("全新".equals(bp_new))? "checked" : ""%>>全新</label>
							...
							<div class="form-group">
								...
								<input type="TEXT" name="bp_loc" size="45"
								 value="<%=bpVO.getBp_loc()%>"class="form-control"  id="jj" style="border-radius: 15px !important">
							</div>
							<div class="display">
						        <span id="panel">請輸地址或地點關鍵字<input type="text" id="keyword" style="border-radius: 15px !important ; width:auto"></span>
						        <div id="map"></div>
						    </div>
						</div>
					</div>
				...
				<div class="col-md-6">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							<div>
								<label>上傳圖1:</label>
								<div class="form-control" style="border-radius: 15px !important"><input type="file" id="bp_pic1" name="bp_pic1"
									class="myFile"></div>
							</div>
							<div>
								<label>上傳圖2:</label>
								<div class="form-control" style="border-radius: 15px !important"><input type="file" id="bp_pic2" name="bp_pic2"
									class="myFile"></div>
							</div>
							<div>
								<label>預覽內容:</label>
								<div>
									<div id="preview">
										<img 
										src="<%=request.getContextPath()%>/bp-front/DBG?bp_no=${bpVO.bp_no}&image_column=bp_pic1"
										id="gro_pic_preview1" style=max-width:100% > 
										<img
										src="<%=request.getContextPath()%>/bp-front/DBG?bp_no=${bpVO.bp_no}&image_column=bp_pic2"
